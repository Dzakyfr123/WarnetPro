    ...
    def _poll_status(self):
        ...
        if session:
            ...
            if self.lock_screen.is_visible() and not self.is_locked:
                self.log('🔓 Sesi aktif terdeteksi. Membuka kunci PC.')
            ...
            if prev_session is not None:
                self.log('Sesi berakhir. PC dikunci.')
                self.warning_played = False
                self.time_up_played = False
            self.last_remaining_secs = None
            ...
            if not self.lock_screen.is_visible():
                self.log('🔒 PC dikunci karena tidak ada sesi aktif.')
                self.root.after(0, self.lock_screen.show)

            def _upd():
                self.lbl_session_name.config(text='PC Terkunci', fg='#ef4444')
